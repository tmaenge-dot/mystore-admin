...
export function getBrand(storeId){
  return readJSON('brand_' + storeId) || null;
}

export function setBrand(storeId, brand){
  // brand: { brandColor, logo }
  writeJSON('brand_' + storeId, brand || {});
  try{ if (process.env.DEBUG_LOGS === '1') fs.appendFileSync(path.join(DB_DIR, 'db_debug.log'), JSON.stringify({ action: 'setBrand', storeId, brand, ts: new Date().toISOString() }) + '\n'); }catch(e){}
  return brand || {};
}

export default { getCart, saveCart, listOrders, addOrder, getOrder, deleteCart, deleteOrder, auditLog, getPromo, setPromo, getImageMap, setImageMap, getBrand, setBrand };
